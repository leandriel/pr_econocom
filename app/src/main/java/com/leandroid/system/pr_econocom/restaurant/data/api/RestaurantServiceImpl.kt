...
class RestaurantServiceImpl (private val client: HttpClient) : RestaurantService {
    ...
    override suspend fun getRestaurantData(latitude: Double, longitude: Double): ApiResponseData<List<Restaurant>> {
        val url = buildUrl(latitude, longitude)
        return client.get(url).body<ApiResponseData<List<Restaurant>>>()
    }
