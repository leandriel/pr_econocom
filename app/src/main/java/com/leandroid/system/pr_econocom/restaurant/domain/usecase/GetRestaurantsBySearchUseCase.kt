...
class GetRestaurantsBySearchUseCase(private val repository: RestaurantRepository) :
    BaseUseCase<String, Flow<ResponseStatus<List<Restaurant>>>> {

    override suspend fun execute(param: String): Flow<ResponseStatus<List<Restaurant>>> = flow {
        emit(ResponseStatus.Loading(true))
        val result = repository.getRestaurantsBySearch(param)
        emit(result)
